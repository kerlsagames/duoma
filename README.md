...
4. Optional, two real phones: create a free [Supabase](https://supabase.com) project, run `001` through `019` in the SQL editor (do not stop at 007 — Help → admin Feedback needs `011`, two-phone check-ins and pose asks need `012`, a new phone needs `013`/`019`, admin couple drill-down needs `014`, the passphrase directory needs `015`, Written homemade cards/dares/bets need `016`, the passphrase seeing Feedback / birthdays / in-app time needs `018`), then set `EXPO_PUBLIC_SUPABASE_URL`, `EXPO_PUBLIC_SUPABASE_ANON_KEY`, and (server-only) `SUPABASE_SERVICE_ROLE_KEY`. After you sign in once, mark your profile admin with `update public.profiles set is_admin = true where lower(email) = 'you@email';`. The Vercel cron `0 18 * * *` hits `/api/push/daily` so both lock screens get the curiosity question while the app is closed.
