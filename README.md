...
- **Home** — Daily Check-In, Calendar, four hubs, and a dotted favorites strip. Shared world is off until you add it in Home settings (cog on the bottom bar). Stats and badges sit on the bar-chart button to the right of Home.
...
- **Desire** — Get Spicy, Dare Me (18 categories, 200 dares), Roleplays (cartoon stills under each scene), Positions (save to To-do, tick off, ask them tonight), The How (28 techniques in four parts: who does what, on which part of the body, a timed try, body-word translations, optional science speak), Fantasy Matcher (swipe deck, To-do, Completed, Passed), Intimacy Streak (the fire grows with dares, spicy nights, pings & Connect), The Sexy Vault (shared pin, photo/video, optional hide-until).
- **Fun** — Chicken (300 silly dares — send one, they cluck or commit, eggs on the board), Coupons, How Well Do You Know Me (20 themed card packs — rip yours, guess theirs, next pack stays sealed), LoveBetz (propose a slip, they accept), Photo Memory (one weekly shot, shuffle until you lock it, clothesline gallery), Draw It, Scoreboard, Daily Word, Fair Share (spin a chore or tap who did it last).
- **Home Base** — Groceries & Errands, Birthdays (family & friends, onto the home calendar), Gifts (wish lists, what to buy, and a ledger of what people got each Christmas, birthday, anniversary), Meal Plan (post-it week, regulars, feeling lucky), Travel itinerary, Shared Goals (long-term on top, short-term list), Shared Budget (weekly or fortnight pay, bills, spending), Household maintenance, Emergency vault, Period Tracker. Each hub opens as two columns; the cog can hide, reorder, or switch to a list. Couple settings (notifications, card bank, pair code, sign out) live on the Home cog.
- **App cogs** — Every hub app has a settings cog. Colour and type size are always there (same idea as meal-plan post-its). Each app also has its own extras — jumbo Discover cards, discreet Period title, glance-mode Budget, earned-only Scoreboard, hide After dark on the mixtape, and so on. Hub colour still paints the Home tile.
